...
if ($raw === false || trim($raw) === "") {
  respond(false, "Empty request body");
}
...
if ($pump_name === "" || $location === "") {
  respond(false, "pump_name and location required");
}
...
$checkSql = "
  SELECT 1 
  FROM pump 
  WHERE user_id = ? AND pump_name = ?
  LIMIT 1
";

$checkStmt = $conn->prepare($checkSql);
if (!$checkStmt) {
  respond(false, "Prepare failed (check): " . $conn->error);
}
...
if ($checkRes && $checkRes->num_rows > 0) {
  $checkStmt->close();
  respond(false, "This Device Name already exists for your account.");
}
$checkStmt->close();


$insSql = "
  INSERT INTO pump (user_id, pump_name, location)
  VALUES (?, ?, ?)
";

$insStmt = $conn->prepare($insSql);
if (!$insStmt) {
  respond(false, "Prepare failed (insert): " . $conn->error);
}
